  ...
  <div class="app-container">
    ...
    <div class="tools-div">
      <el-button type="success"
                 ...
                 size="mini"
                 @click="add()"
                 :disabled="$hasBP('bnt.processTemplate.templateSet')  === false">添加审批设置</el-button>
    </div>
    <!-- 列表 -->
    <el-table v-loading="listLoading"
      ...
      <el-table-column prop="processTypeName"
                       label="审批类型" />
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button type="text"
                     size="mini"
                     @click="show(scope.row)">查看审批设置</el-button>
          <el-button type="text"
    ...
    <el-dialog title="查看审批设置"
               :visible.sync="formDialogVisible"
               width="35%">
      <h3>基本信息</h3>
      <el-divider />
      <el-form ref="flashPromotionForm"
               label-width="150px"
               size="small"
               style="padding-right: 40px;">
        <el-form-item label="审批类型"
                      style="margin-bottom: 0px;">{{ processTemplate.processTypeName }}</el-form-item>
        <el-form-item label="名称"
                      style="margin-bottom: 0px;">{{ processTemplate.name }}</el-form-item>
        <el-form-item label="创建时间"
                      style="margin-bottom: 0px;">{{ processTemplate.createTime }}</el-form-item>
      </el-form>
      <h3>表单信息</h3>
      <el-divider />
      <div>
...
export default {
  ...
  methods: {
    ...
    publish (id) {
      this.$confirm('此操作将发布模板到数据库上, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => { // promise
